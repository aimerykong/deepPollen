...
gpuDevice(gpuId);
%% setup network
saveFolder = 'main006_clsTwoViews_v1_aboveRes4_sumConf';
modelName = 'softmax_net-epoch-19.mat';
...
outputF = fopen('model3_fused_catatumbus_part2.csv', 'w');
...
for i = 1:length(imdb.(setName).view1) 
    ...
    view1Batch = imread(fullfile(imdb.root_path, imdb.datasetFolder, 'S_catatumbus_eocene_SA_selected', path4view1));
    view1Batch = repmat(view1Batch,[1 1 3]);
    view1Batch = bsxfun(@minus, single(view1Batch), mean_rgb) ;
    
    path4view2List = imdb.(setName).view2{i};
    view2Batch = zeros(640, 640, 3, length(path4view2List), 'single');
    for jj = 1:length(path4view2List)
        ...
        view2Batch(:,:,:,jj) = cur_image;
    end    
    inputs = {'data', gpuArray(cat(4,view1Batch,view2Batch))};
    ...
    txtLine{end+1} = sprintf('%02d name:%s \t\t pred-genus: %s (%.3f)', i, path4view1, imdbRef.meta.genusName{predClass}, probVal/2);
    fprintf('%s\n',txtLine{end});
    
    fprintf(outputF, '%s, ', path4view1 );
    for j = 1:length(imdbRef.meta.genusName)    
        fprintf(outputF, '%.3f', SoftMaxLayer(j)/2);
...
%% save
save(sprintf('./result/fossilPart2_view12_%s',saveToken), 'txtLine', 'predList', 'confMat');
%% leaving blank
%{
01 name:S_catatumbus_max2_M119_T58_2_Airyscan Processing_mz.png                  pred-genus: Crudia (0.853)
02 name:S_catatumbus_max2_M120_V24_4_Airyscan Processing_mz.png                  pred-genus: Anthonotha (0.497)
03 name:S_catatumbus_max2_M128_J49_1_Airyscan Processing_mz.png                  pred-genus: Crudia (0.459)
04 name:S_catatumbus_max2_M129_R55_4_Airyscan Processing_mz.png                  pred-genus: Crudia (0.674)
05 name:S_catatumbus_max2_M129_S51_4_Airyscan Processing_mz.png                  pred-genus: Crudia (0.904)
06 name:S_catatumbus_max2_M50_W31_1_Airyscan Processing_mz.png           pred-genus: Crudia (0.517)
%}
